...
data Str = Nil | S String  deriving Show 

wordToken s = wordTokenH s ""

trans (h:t) = h

wordTokenH [] _ = []
wordTokenH (h:t) acc
     | (h == ' ') && acc /= "" = ([acc] ++ (wordTokenH t "")) 
     | elem h punct = ( [acc] ++ [ [h] ] ++(wordTokenH t ""))
     | otherwise = wordTokenH t added
     where added = acc ++ [h] 
...
sentToken s = sentTokenH " " (wordToken s)

sentTokenH acc [] = []

sentTokenH acc (h:t) 
    | matchEnd h = ( ( acc ++  h ) :  ( sentTokenH "" t) ) 
	| matchPunct h  = sentTokenH (acc ++ h) t 
	| otherwise = sentTokenH (acc ++ " " ++ h) t 
	
        
		

add  [] = []				
add  (h:t) = if ( not( elem h punct)) then h: add t 
                else " " ++ [h] ++ " " ++ add t 

--wordToken :: [Char] -> [[Char]]
--wordToken l = words  (add l)
...
generateBiPairs[_] = []							
generateBiPairs(h:t) =  [( h , head t )] ++ generateBiPairs t  						
	

generateTriPairs [_,_] = []
generateTriPairs(h:t) = [ (h,head t, head ( tail t ) ) ] ++ generateTriPairs t 
...
count a c (h:t) 
                   | a == h = count a (c+1) t 
				   | otherwise = count a c t 
				   
				   
-- Map Occurence on a List of Unique Bigram or Trigrams 		  
map2 f [] _ _ = []
map2 f (h:t) 0 l = ( f h 0 l ) : map2 f t 0 l


-- count of all elements 
occur l l2 = map2 count l 0 l2
	
	
	
-- pair 
pair  [] [] = []
pair (h:t) (s:d) =  ( (h,s) : pair t d )
...
uniqueBigrams l = dups ( generateBiPairs l )

uniqueTrigrams :: [String] -> [(String,String,String)]

uniqueTrigrams l = dups ( generateTriPairs l )
...
bigramsFreq l = ( pair (uniqueBigrams l) (occur (uniqueBigrams l) (generateBiPairs l) ) )

trigramsFreq:: Num a => [String] -> [((String,String,String),a)]
trigramsFreq l =( pair (uniqueTrigrams l) (occur (uniqueTrigrams l) (generateTriPairs l) ))
...
spPair [] [] = [] 
spPair (h:t) (s:d) = ( ( (fst h) , s) : spPair t d )

-- genProbPairs
--genProbPairs [] _ = []
genProbPairs :: Fractional a => [((String,String,String),a)] ->[((String,String),a)] -> [((String,String,String),a)]

genProbPairs l1 l2 = spPair l1 (map3  generateOneProb l1 l2)



map3 f [] _ = []
map3 f (h:t) l2 = ( f h l2) : map3 f t l2
...
filter2 f a (h:t) = if (f a h ) then h:filter2 f a t 
                   else filter2 f a t 


find _ _ [] = error( "Sorry, it is not possible to infer from current database")

find count target (h:t) 
        | count == target = h
		| otherwise = find (count+1) target t 

transform [a,b] = ( a,b )

third ( ( _ , _ , x ) , _ ) = x 

--generateNextWord l1 l2  =  third (    ( find 0   randomZeroToX ( length(filter2 (check) l1 l2 )  )   (filter2 (check) l1 l2) ) )

generateNextWord :: (Ord a, Fractional a) => [String] ->[((String,String,String),a)] -> String

generateNextWord l1 l2 = third ( find2 l1 l2 )

filter3 l1 l2 = filter2 (check) l1 l2 
...
--last2 [ x,y] = [x,y]
--last2 ( x,y :t ) = last2 (y:t)



last2 l = [ last ( init l ) , last l ] 
